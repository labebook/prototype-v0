...
export default function ModuleDetailPage({ params }: { params: { slug: string } }) {
  const [variation, setVariation] = useState("suspension")

  const [selectedPills, setSelectedPills] = useState<Record<string, boolean>>({
    // Sample Preparation
    "harvest-method-1": false,
    "harvest-method-2": false,
    "wash-pbs": false,
    "wash-tbs": false,
    "storage-not-required": false,

    // Lysis Buffer Preparation
    "buffer-np40": false,
    "buffer-ripa": false,
    "buffer-urea": false,
    "protease-not-required": false,
    "phosphatase-not-required": false,
    "nuclease-none": false,
    "nuclease-dnase": false,
    "nuclease-rnase": false,
    "nuclease-both": false,
    "chelator-not-required": false,

    // Principal Work
    "lysis-chemical": false,
    "lysis-homogenization": false,
    "lysis-sonication": false,
    "clarify-crude": false,
    "clarify-precleared": false,
    "clarify-filtered": false,
    "clarify-highspeed": false,

    // Storage
    "aliquot-not-required": false,
    "snapfreeze-not-required": false,

    "harvest-trypsin": false,
    "harvest-scraping": false,
    "harvest-edta": false,
  })
  ...
  const selectedCount = Object.values(selectedPills).filter(Boolean).length
    ...
    return (
      <div className="flex items-center flex-nowrap overflow-x-auto py-3 border-b border-gray-100 last:border-b-0">
        <div className="font-medium text-base whitespace-nowrap mr-4 flex items-center gap-2">
          <span>{number}.</span>
          <span>{title}</span>
          {isOptional && <span className="text-xs text-gray-500 italic">(optional)</span>}
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
  ...
  return (
    <>
      ...
      <div className="flex items-end justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold mb-1 uppercase tracking-wide">
            Whole-Cell Protein Lysate Preparation Using Detergent-Based Buffer
          </h1>
          <p className="text-base text-gray-600">Protocol Plan</p>
        </div>
        <Link href={`/custom-modules/${params.slug}/example?variation=${variation}`}>
          <Button className="h-10 px-4 bg-black hover:bg-gray-800 text-white">View Protocol</Button>
        </Link>
      </div>

      {/* Main Content - Module Steps */}
      <div className="mb-8 bg-white border border-[#E5E7EB] rounded-lg">
        <div className="px-6 pt-8 pb-6">
          <h3 className="text-xl font-medium mb-2">Module Steps</h3>
          <p className="text-sm text-gray-600 mb-6">Select options to customize your module protocol</p>
          ...
          <div className="mb-8">
            <Select value={variation} onValueChange={setVariation}>
              ...
              <SelectContent>
                ...
                <SelectItem value="adhesive">Adhesive Cells</SelectItem>
              </SelectContent>
            </Select>
          </div>
      ...
      <div className="flex justify-center mt-8">
        <Link href={`/custom-modules/${params.slug}/example?variation=${variation}`}>
          <Button className="h-12 px-8 bg-black hover:bg-gray-800 text-white text-base">View Protocol</Button>
        </Link>
      </div>
    </>
  )
